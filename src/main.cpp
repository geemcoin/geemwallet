...
int main(int argc, char* argv[]) {
  ...
  QSettings geemKey("HKEY_CLASSES_ROOT\\geem", QSettings::NativeFormat);
  geemKey.setValue(".", "Geem Wallet");
  geemKey.setValue("URL Protocol", "");
  QSettings geemOpenKey("HKEY_CLASSES_ROOT\\geem\\shell\\open\\command", QSettings::NativeFormat);
  geemOpenKey.setValue(".", "\"" + QCoreApplication::applicationFilePath().replace("/", "\\") + "\" \"%1\"");
#endif
  ...
  //as root
  args << "-c" << "printf '[Desktop Entry]\\nName = Geem URL Handler\\nGenericName = Geem\\nComment = Handle URL Sheme geem://\\nExec = " + QCoreApplication::applicationFilePath() + " %%u\\nTerminal = false\\nType = Application\\nMimeType = x-scheme-handler/geem;\\nIcon = Geem-Wallet' | tee /usr/share/applications/geem-handler.desktop";
